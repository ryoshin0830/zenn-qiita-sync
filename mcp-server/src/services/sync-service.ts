...
export class ZennQiitaSyncService {
  private previewProcesses: Map<string, any> = new Map();
   ...
   */
  async postArticle(zennSlug: string, updateIfExists: boolean = false): Promise<PostResult> {
    // Read Zenn article
   ...
   */
  async syncAllArticles(forceUpdate: boolean = false): Promise<SyncResult> {
   ...
   */
  async pullArticles(platform: 'qiita' | 'both' = 'qiita'): Promise<PullResult> {
   ...
   */
  async startPreview(platform: 'zenn' | 'qiita'): Promise<PreviewResult> {
   ...
   * Get sync mapping
   */
  async getSyncMapping(): Promise<Record<string, any>> {
   ...
   */
  async publishZennArticles(commitMessage?: string, push: boolean = true): Promise<{ message: string }> {
    try {
